...
```mermaid
flowchart LR
  user["用户访问页面"] --> edge{"CDN / 缓存<br/>能直接回答吗？"}
  edge -->|"能"| cached["返回缓存的 HTML / JSON"]
  edge -->|"不能"| next["进入 Next.js 服务器"]

  next --> html{"第一份 HTML<br/>在哪里生成？"}
  html -->|"浏览器 JS 生成"| csr["CSR"]
  html -->|"请求时服务器生成"| ssr["SSR"]
  html -->|"构建时提前生成"| ssg["SSG / 静态化"]

  csr --> jsRuntime["浏览器运行 JS"]
  ssr --> browser["浏览器显示 HTML"]
  ssg --> browser
  cached --> browser
  jsRuntime --> reactHydrate["React hydration<br/>接管交互"]
  browser --> reactHydrate
```
...
flowchart LR
  subgraph server["Server"]
    sc["Server Components<br/>取数据 / 读数据库 / 用 secret"]
    rsc["生成 RSC Payload"]
    html["预渲染 HTML"]
    sc --> rsc
    rsc --> html
  ...
  subgraph client["Browser"]
    preview["先显示 HTML"]
    reconcile["用 RSC Payload 协调组件树"]
    hydrateClient["hydrate Client Components"]
    interactive["页面可交互"]
    preview --> reconcile --> hydrateClient --> interactive
  end

  html --> preview
  rsc --> reconcile
  cc["Client Components<br/>useState / onClick / useEffect"] --> hydrateClient
```
